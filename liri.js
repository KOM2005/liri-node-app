		...
		switch (answers.program) {
			...
			case 'spotify-this-song':
				inquirer.prompt([{
				    type: 'input',
				    name: 'song',
				    message: 'What song you want to get info?',
				  }
				])
				.then((answers) => {
...
function reRunProgram() {
	inquirer.prompt([{
	    type: 'confirm',
	    name: 'confirm',
	    message: 'restart the program?',
	  }
	])
	.then((answers) => {
		if (answers.confirm) {
			logStuffThatHappens('reRun the program', 'Yes');
			programStart();
		} else {
			logStuffThatHappens('abort the program', 'No');
			console.log("\nGood Bye!\n");
		}
	})
}
...
function getTweets() {
	...
	const queryUrl = "https://api.twitter.com/1.1/search/tweets.json?q=oleh_kmyta_2005&result_type=recent&count=20";

	client.get(queryUrl, (error, tweets, response) => {

		if (error) {
			logErrors('getTweets()', '@oleh_kmyta_2005');
			console.log(error);
		}

		console.log("\nHere are the latest tweets from @oleh_kmyta_2005: \n");
   		
   		for (var i = 0; i < tweets.statuses.length; i++) {
   			console.log(tweets.statuses[i].created_at.substring(0,19).grey + " - " + tweets.statuses[i].text)
   		}
   		console.log('');	
	});

	logStuffThatHappens('my-tweets           ', '@oleh_kmyta_2005');
...
function getMusic(song) {
	...
	Spotify.search({ type: 'track', query: song, limit: 1 }, (err, data) => {
		...
		console.log("\nGreat choice!\n");
		console.log(`I love '${songName}' by ${artistName}. Wasn't that on the album '${albumName}'?`);
